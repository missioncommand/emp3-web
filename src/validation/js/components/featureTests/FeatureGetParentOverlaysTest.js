...
class FeatureGetParentOverlaysTest extends Component {
  constructor(props) {
    super(props);
  ...
  getParents() {
    ...
    try {
      feature.getParents({
        onSuccess: cbArgs => {
          toastr.success('Retrieved the features parent overlays(' + cbArgs.overlays.length + ')');
          this.setState({
            overlayParents: cbArgs.overlays
          });
        },
        onError: err => {
          toastr.error('Failed to retrieve parent overlays');
          dispatch(addError(err, 'Feature.getParentOverlays'));
        }
      });
    } catch (err) {
      toastr.error(err.message, 'Feature.getParentOverlays: Critical');
    }
  }
...
FeatureGetParentOverlaysTest.propTypes = {
...
export default connect(mapStateToProps)(FeatureGetParentOverlaysTest);
