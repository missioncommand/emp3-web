...
/**
 * Provides the instructions for editing a Square to the editingManager.
...
emp.editors.Square.prototype.addControlPoints = function() {
  ...
  // All features entering into this method should be a Square
  // otherwise this will not work.
  if (this.featureCopy.data.type === 'square') {
...
emp.editors.Square.prototype.removeControlPoints = function() {
...
emp.editors.Square.prototype.startMoveControlPoint = function(featureId, pointer) {
  ...
  // from the last point in the square to the first point.
...
emp.editors.Square.prototype.moveControlPoint = function(featureId, pointer) {
  ...
  // to move the add points in between the first and last vertices of the square.
  if (back === null) {
  ...
  // This is the first point
  //
  // If the vertex we are moving is the first point of the square, then
  // the first point on the animation is the last point of the square
  if (currentVertex.before === null) {
    animationCoordinates.push(this.vertices.tail.before.feature.data.coordinates);
  } else {
    animationCoordinates.push(currentVertex.before.before.feature.data.coordinates);
  ...
  //
  // if the vertex we are moving is the last point of the square, the third
  // animation point will be the first point of the square.
  if (currentVertex.next.next === null) {
    animationCoordinates.push(this.vertices.head.feature.data.coordinates);
  } else {
    animationCoordinates.push(currentVertex.next.next.feature.data.coordinates);
