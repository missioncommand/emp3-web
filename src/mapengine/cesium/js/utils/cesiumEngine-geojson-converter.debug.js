...
cesiumEngine.geojsonParser = (function () {
    ...
    */
    ...
    function addGeoJsonPoint(geojson, name, id, overlayId, description, modifiers, zoom) {
        var coordinates,
            iconUrl,
            xOffset,
            yOffset,
        ...
        iconUrl = modifiers.iconUrl || emp.imagesUrls.whtBlank;
        fillColor = modifiers.fillColor || "FF000000";
        xOffset = modifiers.xOffset || 32;
        yOffset = modifiers.yOffset || 1;
        xUnits = modifiers.xUnits || "pixels";
        yUnits = modifiers.yUnits || "pixels";
        name = name || "";

        fillColor = cesiumEngine.utils.adjustColor(fillColor);
    ...
    function addGeoJsonLineString(geojson, name, id, overlayId,
        description, modifiers, zoom) {
        var result = "",
            coordinateString = "",
            coordinates,
            kml,
        ...
        modifiers = modifiers || {};
        lineColor = modifiers.lineColor || 'FF0000FF';
        lineThickness = modifiers.lineThickness || 3.0;

        lineColor = cesiumEngine.utils.adjustColor(lineColor);
            ...
            for (coordIdx = 0; coordIdx < coordinates.length; coordIdx += 1) {
                ...
                if (latlon !== undefined && latlon !== null) {
                    ...
                    if (latlon.length >= 2) {
                        ...
                        if (latlon.length === 3) {
                            coordinateString = coordinateString + lon + "," + lat + "," + coordinates[2];
    ...
    function addGeoJsonPolygon(geojson, name, id, overlayId, description,
        modifiers, zoom) {
        ...
        if ((geojson.coordinates !== undefined || geojson.coordinates !== null) && geojson.coordinates.length > 0) {
            ...
            modifiers = modifiers || {};
            fillColor = modifiers.fillColor || "88000000";
    ...
    */
    ...
    function addGeoJsonMultiPoint(geojson, name, id, overlayId,
        description, modifiers, zoom) {
        var iconUrl,
            coordinates,
            batch,
        ...
        if (geojson.coordinates !== undefined || geojson.coordinates !== null) {
            ...
            modifiers = modifiers || {};
            fillColor = modifiers.fillColor || "FFFFFFFF";
            iconUrl = modifiers.iconUrl || emp.imagesUrls.whtBlank;
            xOffset = modifiers.xOffset || 32;
            yOffset = modifiers.yOffset || 1;
            xUnits = modifiers.xUnits || "pixels";
    ...
    */
    ...
    function addGeoJsonMultiLineString(geojson, name, id, overlayId,
        description, modifiers, zoom) {
    ...
    function addGeoJsonMultiPolygon(geojson, name, id, overlayId,
        description, modifiers, zoom) {
    ...
    */
    ...
    function addGeoJsonFeature(geojson, name, id, overlayId, description, modifiers, zoom) {
    ...
    function addGeoJsonFeatureCollection(geojson, name, id, overlayId, description, modifiers, zoom) {
        ...
        if (geojson !== null && geojson.type === "FeatureCollection") {
            ...
            if (features !== null) {
                ...
                for (i = 0; i < len; i += 1) {
                    ...
                    result = addGeoJsonFeature(feature, name + " " + i, id, overlayId, description, modifiers, zoom);
                    if (result.success) {
                        obj = {
                            kml: result.data,
                            name: name,
     ...
     * polygon, or another geomtryCollection.  All graphics within the geometry
     * Collection are realted and grouped under an containing effort.  Names of each
    ...
    function addGeoJsonGeometryCollection(geojson, name, id, overlayId, description, modifiers, zoom) {
    ...
    return {
        /*
        Converts an array of geojson objects to kml.  The returned object that can 
        ...
        @param {array} batch - an array of geojson objects that are of the format:
        { geojson: (string), // geojson object
        name: (string),  // the display name of the object
        id: (string),    // the id of the object
        overlayId: (string), // the folder the object should be in
        description: (string), // an description of the object, possibly in html
        modifiers: (Modifiers) // a Modifiers object that can have color, line, point and polygon information
        }
        */
        geojsonToKmlBatch: function (batch) {
            ...
            for (i = 0; i < length; i += 1) {
                try {
                    ...
                    name = batch[i].name || "";
                    id = batch[i].id;
                    overlayId = batch[i].overlayId || "";
                    description = batch[i].description || "";
                    modifiers = batch[i].properties || {}; //changed modifier to properties
                    ...
                    if (geojson.type !== undefined || geojson.type !== null) {
                        ...
                        switch (type.toLowerCase()) {
                        case "point":
                            result = addGeoJsonPoint(geojson, name, id, overlayId,
                                description, modifiers, zoom);
                            break;
                        case "linestring":
                            result = addGeoJsonLineString(geojson, name, id, overlayId,
                                description, modifiers, zoom);
                            break;
                        case "polygon":
                            result = addGeoJsonPolygon(geojson, name, id, overlayId,
                                description, modifiers, zoom);
                            break;
                        case "multipoint":
                            result = addGeoJsonMultiPoint(geojson, name, id, overlayId,
                                description, modifiers, zoom);
                            break;
                        case "multilinestring":
                            result = addGeoJsonMultiLineString(geojson, name, id, overlayId,
                                description, modifiers, zoom);
                            break;
                        case "multipolygon":
                            result = addGeoJsonMultiPolygon(geojson, name, id, overlayId,
                                description, modifiers, zoom);
                            break;
                        case "geometrycollection":
                            result = addGeoJsonGeometryCollection(geojson, name, id, overlayId,
                                description, modifiers, zoom);
                            break;
                        case "feature":
                            result = addGeoJsonFeature(geojson, name, id, overlayId,
                                description, modifiers, zoom);
                            break;
                        case "featurecollection":
                            result = addGeoJsonFeatureCollection(geojson, name, id, overlayId,
                                description, modifiers, zoom);
                            break;
                        default:
                            result = {
                                success: false,
                                message: "Unssupported GeoJson type"
                            };
                            break;
                        }
                        ...
                        result = {
                            success: false,
                            message: "Incorrectly formated GeoJson"
                        };
                        // Since this is a batch call, only report the first error that was found. Maintain
                        // a separate variable for overall result
                        if (overallResult === null) {
        ...
        */
        geojsonToKml: function (geojson, name, id, overlayId, description, modifiers) {
            var batch = [],
                obj = {},
                result;
