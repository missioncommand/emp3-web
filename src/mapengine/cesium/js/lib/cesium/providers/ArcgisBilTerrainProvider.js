	...
	OGCHelper.WMSParser.getMetaDatafromXML = function(xml, description) {
		...
		if (Cesium.defined(resultat.formatImage)
			...
			resultat.formatImage.terrainDataStructure = {
				...
				elementMultiplier : 256.0,
                                isBigEndian : false
				//ace commented out isBigEndian : true
			};
		} else {
			resultat.formatImage = undefined;
		}
		var layerNodes = xml
				.querySelectorAll("Layer,Layer[queryable='true']");
//ace                        var layerNodes = xml
//				.querySelectorAll("Layer[queryable='1'],Layer[queryable='true']");
	 ...
	 * 
	 * @alias ArcgisBilTerrainProvider
	 * @constructor
	 * 
	 * @param {String}
	 ...
	 * @see TerrainProvider
	 */
	var ArcgisBilTerrainProvider = function ArcgisBilTerrainProvider(
			description) {
	...
	*
	*/
	ArcgisBilTerrainProvider.arrayToHeightmapTerrainData=function(arrayBuffer,limitations,size,formatArray,hasWaterMask,childrenMask){
...
/**
	...
	* childrenMask: 			Number defining the childrenMask
	*/
	ArcgisBilTerrainProvider.imageToHeightmapTerrainData=function(image,limitations,size,hasWaterMask,childrenMask,hasStyledImage){
		if(typeof(size)=="number"){
			size={width:size,height:size};
		}
						...
						if(!isNaN(x+y+level)){
							...
							if (Cesium.defined(promise)) {
								retour = Cesium.when(promise,function(image){
											return ArcgisBilTerrainProvider.imageToHeightmapTerrainData(image,limitations,
					        ...
					        if (Cesium.defined(promise)) {
								retour = Cesium.when(promise,
													function(arrayBuffer) {
														return ArcgisBilTerrainProvider.arrayToHeightmapTerrainData(arrayBuffer,limitations,
	...
	Cesium.ArcgisBilTerrainProvider = ArcgisBilTerrainProvider;
})();