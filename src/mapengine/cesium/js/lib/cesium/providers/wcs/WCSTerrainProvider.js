...
(function () {
    ...
    OGCHelper.WCSParser.getDescribeCoverage = function (coverage, description) {
        ...
        //var mntBbox = "&SUBSET=Long,EPSG:4326(" + lowerCorner[0] + "," + upperCorner[0] + ")&SUBSET=Lat,EPSG:4326(" + + lowerCorner[1] + "," + upperCorner[1] + ")";
       var mntBbox = "&SUBSET=Long,EPSG:4326({west},{east})&SUBSET=Lat,EPSG:4326({south},{north})";
    ...
    function templateToURL(urlParam, x, y, level, provider) {
       ...
       // urlParam = urlParam.replace(/%3A/g, ":").replace(/%2F/g, "/").replace(/%3D/g, "=").replace(/%3F/g, "?").replace(/%26/g, "&").replace(/%2C/g, ",").replace(/%7B/g, "{").replace(/%7D/g, "}");
        urlParam = urlParam.replace(/%7B/g, "{").replace(/%7D/g, "}");
