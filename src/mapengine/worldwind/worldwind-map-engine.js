...
emp.engineDefs.worldWindMapEngine = function (args)
{
    ...
    engineInterface.map.config = function (transaction)
    {
        ...
        //optimization start
        if (EMPWorldWind.utils.defined(config.renderingOptimization) && (config.renderingOptimization !== empWorldWind.enableRenderingOptimization))
        {
            bRangeChanged = true;
            empWorldWind.enableRenderingOptimization = config.renderingOptimization;
        }
        if (EMPWorldWind.utils.defined(config.midDistanceThreshold) && (config.midDistanceThreshold !== empWorldWind.singlePointAltitudeRanges.mid))
        {
            bRangeChanged = true;
            empWorldWind.singlePointAltitudeRanges.mid = config.midDistanceThreshold;
        }
        if (EMPWorldWind.utils.defined(config.farDistanceThreshold) && (config.farDistanceThreshold !== empWorldWind.singlePointAltitudeRanges.high))
        {
            bRangeChanged = true;
            empWorldWind.singlePointAltitudeRanges.high = config.farDistanceThreshold;
